...
async def on_message(message):
    ...
    if (message.content).startswith(prefix):
        ...
        if (re.search(r".*about.*", message.content)!=None):
            ...
            response+= "- Every aspect of eclipse 1v1 battles except rift canon, antimatter splitter and regen.\n"
            ...
            response+= "- Survival graph: returns the probability of each ship making it out alive (it's a little buggy atm).\n"
            response+= "- Medium performance: can tackle most battles but not really large ones.\n"
            response+= "**Roadmap:**\n"
